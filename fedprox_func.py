...
def FedSTS(
    ...
    """
    Algorithm 3: FedSTS main function
    Implements the complete FedSTS algorithm
    """
    K = len(training_sets)  # number of clients
    n_samples = np.array([len(db.dataset) for db in training_sets])
    weights = n_samples / np.sum(n_samples)
    ...
    loss_hist = np.zeros((n_iter + 1, K))
    acc_hist = np.zeros((n_iter + 1, K))
    metrics_record = np.zeros((n_iter + 1, K))
    ...
    for i in range(n_iter):
        ...
        for k in range(K):
            ...
            # Local update
            local_learning(local_model, mu, local_optimizer, training_sets[k], 1, loss_classifier)
            
            # Calculate gradient
            grad = []
            for param1, param2 in zip(local_model.parameters(), model.parameters()):
                grad.append((param1 - param2).data)
            
            # Flatten and store gradient
            flat_grad = torch.cat([g.flatten() for g in grad])
            client_gradients.append(flat_grad)
            original_shapes.append([g.shape for g in grad])
        
        # Compress gradients
        d_prime = max(int(client_gradients[0].shape[0] * compression_ratio), 1)
        compressed_gradients = []
        group_indices_list = []
        
        for grad in client_gradients:
            compressed, group_indices = information_squeeze(grad, d_prime)
            compressed_gradients.append(compressed)
            group_indices_list.append(group_indices)
        
        # Perform client stratification
        strata = client_stratification(compressed_gradients, num_strata)
        
        # Calculate sampling allocation
        allocations = calculate_allocation(strata, compressed_gradients, n_sampled)
        
        # Sample clients from each stratum
        selected_clients = []
        for stratum, allocation in zip(strata, allocations):
            if not stratum or allocation == 0:
                continue
                
            # Calculate importance weights within stratum
            stratum_grads = torch.stack([compressed_gradients[j] for j in stratum])
            weights = torch.norm(stratum_grads, dim=1)
            probs = weights / weights.sum()
            
            # Sample clients
            num_to_sample = min(allocation, len(stratum))
            if num_to_sample > 0:
        ...
        for idx, client_idx in enumerate(selected_clients):
            ...
            local_learning(
                ...
                mu,
                local_optimizer,
                training_sets[client_idx],
                n_SGD,
                loss_classifier,
            )
            
            # Add weighted contribution
            for global_param, local_param in zip(new_model.parameters(), local_model.parameters()):
                global_param.data.add_(local_param.data * client_weights[idx])
